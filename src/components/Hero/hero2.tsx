...
const Hero2 = () => {
  ...
  // Memoize transformation ranges to improve performance
  const [scaleRange, opacityRange, xImagesRange, xWhoWeAreRange, yRange] =
    useMemo(() => {
      return [
        shouldReduceMotion ? [1, 1, 1] : [5, 100, 100],
        shouldReduceMotion ? [1, 1, 1] : [0, 0, 1],
        shouldReduceMotion ? [0, 0] : [0, -55],
        shouldReduceMotion ? [0, 0] : [0, -38],
        shouldReduceMotion ? [0, 0, 0, 0] : [0, -150, 0, 750],
      ];
    }, [shouldReduceMotion]);

  // Apply transformations
  const scale = useTransform(scrollY, [0, 350, 1200], scaleRange);
  const opacity = useTransform(scrollY, [0, 200, 400], opacityRange);
  const xImages = useTransform(scrollY, [200, 800], xImagesRange);
  const xWhoWeAre = useTransform(scrollY, [200, 900], xWhoWeAreRange);
  const y = useTransform(scrollY, [0, 50, 300, 1200], yRange);
